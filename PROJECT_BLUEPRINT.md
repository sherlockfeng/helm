...
export interface KnowledgeContext {
  hostSessionId: string;
  cwd: string;
  filePath?: string;       // 可选：当前编辑/讨论的文件
  recentPrompts?: string[]; // 可选：最近用户输入，用于检索 hint
}
...
export interface KnowledgeProvider {
  ...
  /** 在 host_session_start 时调用，返回注入到 additional_context 的 markdown */
  ...
  /** 健康检查，UI 在 Settings 显示状态用 */
  healthcheck(): Promise<{ ok: boolean; reason?: string }>;
}
```

### 11.5.2 内置 / Reference Provider

`LocalRolesProvider`（内置，必启）

- 包装 `roles` + `knowledge_chunks` 表的检索
- `canHandle` 总是 true（fallback 角色）
- `getSessionContext` 根据 cwd 命中的 role（用户绑定 chat 到 role 时记录）拉关联 chunks
- `search` 用 cosine similarity（与 `relay/src/roles/library.ts` 行为一致）

`DepscopeProvider`（reference 实现，opt-in）

- 配置：endpoint URL、scm_mappings（cwd glob → scm_name）、auth header（如有）
- `canHandle`：cwd 命中任一 scm_mapping
- `getSessionContext`：
  1. 解析 cwd → scm_name
  2. 拿 file_path（可选），调 depscope `GET /api/specialized/{latest_run}` 过滤该文件
  3. 调 `GET /api/analysis/{latest_run}` 拿 wrappers / 整体覆盖率
  4. 渲染成 markdown：
     ```
     ## TikTok Web 依赖上下文 (depscope @ run #1247)
     当前文件: src/routes/upload/page.tsx → /creator-center/upload
     依赖服务:
       - starling (hard) — import @byted/starling-client
       - mcs (soft)     — import @byted/mcs-client
     已知 wrapper: useRequest, apiClient
     HAR 验证: starling 100%, mcs 30%
     ```
- `search`：通过 depscope 已有的 search/impact 端点；查不到的 fall back 空数组
- 缓存：`(scm_name, run_id, file_path)` → context，TTL 5 min；run_id 单调递增可作 cache buster
- 失败处理：网络错误返回 null，UI 显示降级，不阻塞 hook

### 11.5.3 注入时机

| 时机 | 行为 |
|---|---|
| `host_session_start` | 遍历所有启用 provider 的 `canHandle`，命中的并发调 `getSessionContext`，拼接结果（按 provider 优先级顺序）写入 `additional_context` |
| 用户在 UI 内手动触发 "重新注入 context" | 同上，下一次 prompt 前注入（通过 `host_prompt_submit` 的 user_message 修改） |
| MCP tool 显式调用 | 见 §13.2 新增 tool |
| 长 chat 中 cwd 切换 | 检测 cwd 变化，触发新一轮 `canHandle` |

并发调用 timeout 默认 5s，单 provider 慢/失败不阻塞其他 provider。聚合后总长度 cap（默认 8KB），超出按 provider score 截断。

### 11.5.4 Provider 注册与配置

`config.knowledge.providers` 数组，每项：

```ts
{ id: 'depscope', enabled: true, config: { endpoint, scmMappings, authToken? } }
```

`LocalRolesProvider` 隐式启用，不在 config 中。

UI 在 Settings 提供 "知识源" 标签页，列出已注册 provider，显示 healthcheck 状态，提供添加/启用/禁用/编辑 UI。

### 11.5.5 数据库（缓存）

`knowledge_provider_cache` 表（可选，跨重启 cache）：

```sql
CREATE TABLE IF NOT EXISTS knowledge_provider_cache (
...
export interface AppConfig {
  ...
  knowledge: {
    providers: Array<{
      ...
      enabled: boolean;
      config: Record<string, unknown>; // provider 特定，如 endpoint / scmMappings
    }>;
    sessionContextMaxBytes: number;    // 默认 8192
    perProviderTimeoutMs: number;      // 默认 5000
  };
