...
### Phase 2（不在本文档详细规划）
- **RelayBackend 抽象 + 多 backend 实现**：`DepscopeRelay`（内部零配置）/ `CloudflareTunnelRelay`（开源默认）/ `TailscaleRelay`（详见 §11.6）
- PWA：daemon / UI 拆分，走 `RelayBackend` + Web Push
- 知识 / 需求 bundle 导出导入（团队共享）
- 额外 RemoteChannel adapter（Slack / Telegram）
- 额外 KnowledgeProvider 适配（公司 wiki / 自家 SDK 文档站等）
- Claude Code HostAdapter

### MVP 各阶段顺带带入的 KnowledgeProvider 工作

- MVP-1：定义接口、实现 `LocalRolesProvider`、Settings UI 占位
- MVP-2：补充 §13 中的查询类 MCP tool
- MVP-3：实现 `DepscopeProvider` 作为 reference（ByteDance 内部用户首批受益）
...
## 11.5 KnowledgeProvider 抽象（轻量 v1）

为了让 helm 能把外部知识源（公司内部依赖分析服务、私有 wiki、内部 SDK 文档站等）作为 Cursor chat 的 context 注入，引入 `KnowledgeProvider` 抽象。

### 11.5.1 设计原则（v1）

- **配置全部在 app 内**：用户在 Settings UI 维护 cwd ↔ scope 的映射；不读 `.helm-project` / `rush-scm.json` / `pnpm-workspace.yaml` / `nx.json` / `lerna.json`
- **零项目侵入**：项目仓库无需任何文件改动
- **接口最小**：4 个方法，无 `resolveScopes` 三段式
- **monorepo 不自动发现**：用户在 Settings 中显式声明每个 subspace 的 mapping
- **单 chat 单 scope**：sessionStart 命中一个 mapping 即注入；不做 chat 内跨 scope 的自动切换（v1.5 再加）
- **不为缓存加新表**：内存 + TTL；跨重启失效不影响功能

### 11.5.2 接口

定义在 `src/knowledge/types.ts`：

```ts
export interface KnowledgeContext {
  hostSessionId: string;
  cwd: string;
  filePath?: string;          // hook 流入时填充（v1 仅供 provider 内部细化展示，不触发重注入）
}
...
export interface KnowledgeProvider {
  ...
  /** provider 是否对该 context 可响应 */
  canHandle(ctx: KnowledgeContext): boolean | Promise<boolean>;

  /** sessionStart 时调用，返回注入到 additional_context 的 markdown */
...
### 11.5.3 配置形态（在 `~/.helm/config.json` 内）

每个 provider 自带 `mappings` 数组（cwd 前缀 → 该 provider 内部含义的 "scope"）：
    ...
    "providers": [
      {
        ...
        "enabled": true,
        "config": {
          "endpoint": "http://depscope.byted.org",
          "authToken": "",
          "mappings": [
            {
              "cwdPrefix": "~/monorepo/subspaces/webapp-desktop",
              "scmName": "tiktok/webapp/desktop"
            },
            {
              "cwdPrefix": "~/monorepo/subspaces/creator-center",
              "scmName": "tiktok/creator-center"
            }
          ]
        }
      }
...
不同 provider 的 `mappings.[].<extra>` 字段不同（depscope 用 `scmName`，wiki 用 `spaceKey`，等），但都共享 `cwdPrefix` 字段。

`LocalRolesProvider` 不需要 mappings；其 scope 由 chat 在 UI 中显式绑定到 role（沿用 relay 现有概念）。
...
```ts
// src/knowledge/match.ts
export function longestPrefixMatch<M extends { cwdPrefix: string }>(
  cwd: string,
  mappings: M[]
): M | null {
  return mappings
    .filter(m => cwd.startsWith(expandTilde(m.cwdPrefix)))
    .sort((a, b) => b.cwdPrefix.length - a.cwdPrefix.length)[0] ?? null;
}
```

provider 的 `canHandle` 极简：

```ts
class DepscopeProvider {
  canHandle(ctx) {
    return longestPrefixMatch(ctx.cwd, this.config.mappings) !== null;
  }

  async getSessionContext(ctx) {
    const m = longestPrefixMatch(ctx.cwd, this.config.mappings)!;
    return await this.fetchAndRender(m.scmName, ctx.filePath);
  }
}
```
...
`host_session_start` 一次性流程：

1. 框架构造 `KnowledgeContext { hostSessionId, cwd }`
2. 并发跑所有 enabled provider 的 `canHandle`（200ms 总超时）
3. 命中的并发跑 `getSessionContext`（每 provider 5s 超时）
4. 按 `config.knowledge.providers` 数组顺序拼接结果，每段加 `## <displayName>` 头
5. 总长度 cap（默认 8KB），超出按倒序截断
6. 写入 hook response 的 `additional_context`

慢 / 失败的 provider 不阻塞其他 provider；记 warning log；UI 在 Settings 中标记 unhealthy。

### 11.5.6 v1 不做的事（明确清单）

- ❌ filePath 触发的跨 scope 自动重注入（chat 中漂到另一个 subspace 时不会重新注入）
- ❌ ChatDetail UI 显示"活跃 scope"
- ❌ 自动发现（git remote / package.json / monorepo 工具配置）
- ❌ 项目内 `.helm-project` 文件读取
- ❌ 持久化缓存表

### 11.5.7 后续演进路径（不属 MVP-3）

| 阶段 | 增量 |
|---|---|
| v1 (MVP-3) | App 内 mapping，单 chat 单 scope，sessionStart 一次性注入 |
| v1.5 | filePath-based 跨 scope 切换 + ChatDetail UI 显示活跃 scope |
| v2（团队场景）| `.helm-project` 文件 import/export，团队 commit 共享 mapping |
| v2.5 | git remote 自动发现 → 弹询问"是否启用此 provider" |
| v3 | 持久化缓存表，跨重启加速

## 11.6 RelayBackend 抽象（Phase 2 占位）

> **状态**：一期不实现，仅在 `src/relay/types.ts` 定义接口；二期 PWA / mobile 时落地。

一期 helm 桌面 app 是单机的，远程通道通过 `RemoteChannel`（Lark）旁路；二期增加 PWA / mobile 后需要桌面与移动端之间稳定的双向连接。`RelayBackend` 抽象隔离"具体走哪种通道"。

### 11.6.1 设计目标

- 桌面 app 主动 outbound 连接 relay（避免本机 NAT 暴露）
- 多端订阅：同一用户的 desktop + 1..N mobile 同时在线
- 离线 buffer：mobile 不在线时 relay 临时 buffer 事件，上线后下发
- 推送通知：mobile 不在线时通过 APNs/FCM 唤醒
- 平台无关：协议固定，后端可换
...
export interface RelayBackend {
  id: string;                                  // 'depscope' | 'cloudflare-tunnel' | 'self-hosted'
  start(opts: { deviceId, userId, authToken }): Promise<void>;
  stop(): Promise<void>;

  /** 桌面 → 远端，例如把 hook 事件 / 待审批推到所有订阅 device */
  publish(topic: string, payload: unknown): Promise<void>;

  /** 桌面订阅来自 mobile 的命令（消息注入、审批决策） */
  onCommand(handler: (cmd: RelayCommand) => Promise<RelayResponse>): Unsubscribe;

  /** 触发 push（relay 后端实际打 APNs/FCM） */
  pushNotification(deviceId: string, payload: PushPayload): Promise<void>;

  healthcheck(): Promise<{ ok: boolean; reason?: string }>;
}
```

消息走 WebSocket，topic 包括 `chat.event` / `approval.pending` / `approval.decided` / `cmd.inject_message` / `cmd.start_chat`。

### 11.6.3 计划中的实现

| Backend | 适用 | 实现要点 |
|---|---|---|
| `DepscopeRelay` | ByteDance 内部用户，零配置 | depscope FastAPI 进程内挂 `/relay/*` 路由（WebSocket + 设备配对 + Mongo 持久化），用 TCE SSO 鉴权 |
| `CloudflareTunnelRelay` | 开源默认 | 桌面 app 起 HTTPS server + Web Push，Cloudflare Tunnel 暴露给 PWA |
| `TailscaleRelay` | 隐私偏好用户 | 桌面绑 tailnet IP，PWA 在 Tailscale 网内访问 |
| `SelfHostedRelay` | 团队私有部署 | 用户自建轻量 WebSocket relay，提供 deploy 文档（fly.io / Railway / 自家 K8s） |

### 11.6.4 与 depscope 的边界

如果选择把 relay 挂在 depscope 进程里：

- ✅ 部署层面复用：同一 FastAPI 实例 + 同一 Mongo + 同一 SSO；新增 `relay/` 模块独立 collection（`relay_devices` / `relay_pairings` / `relay_events`）
- ❌ 概念层面隔离：relay 模块不调 depscope 的 ServiceSpec / call_tracer 等业务逻辑；反向亦然
- 分离原则：哪天 depscope 要拆，relay 模块整体迁出，对接口零影响
...
`query_knowledge` — Agent 显式查询所有启用的 KnowledgeProvider（见 §11.5）

```ts
input: { query: string; cwd?: string; filePath?: string; providers?: string[] }
output: { snippets: KnowledgeSnippet[] }   // 按 score 降序聚合
```

`list_knowledge_providers` — Agent 知道有哪些 provider 可查

```ts
output: { providers: Array<{ id, displayName, healthy: boolean, reason? }> }
```

> 上述两个 tool 让 Cursor agent 可以在写代码前主动问 "这个项目我应该先看哪些依赖文档"，与 §11.5.3 的被动 sessionStart 注入形成互补。
...
- 列出所有已注册 KnowledgeProvider（`LocalRolesProvider` 隐式启用且不可删，其他 provider 用户增删）
- 每个 provider 一个详情面板：
  - ☑ 启用 / 禁用开关
  - provider 自身的全局配置（如 endpoint / authToken）
  - **项目映射表**（cwd 前缀 → provider 自定义字段，如 depscope 的 `scmName`）
    - 表格列：本地路径 / scope 标识 / 测试匹配按钮
    - "+ 添加映射"按钮，弹出文件选择器选 cwd
    - 路径支持 `~` 展开
  - 健康状态指示（绿点 / 黄点 / 红点 + 最近一次 healthcheck 时间和原因）
  - "测试连通性"按钮手动触发 healthcheck
...
export interface AppConfig {
  ...
  knowledge: {
    providers: Array<{
      id: string;                       // 'depscope' | 'wiki' | etc
      enabled: boolean;
      config: ProviderConfig;           // provider 自带 schema（含 mappings: { cwdPrefix, ... }[]）
    }>;
    sessionContextMaxBytes: number;     // 默认 8192
    canHandleTotalTimeoutMs: number;    // 默认 200
    getContextTimeoutMs: number;        // 默认 5000
  };
  relay?: {                            // Phase 2 才生效，一期为空
...
首次启动时若 config.json 不存在，写入默认值（`channels.lark.enabled = false`，`knowledge.providers = []`，`relay` 不写入，用户在 Settings 中按需开启）。

DepscopeProvider 的 `ProviderConfig` 实例形态（参考）：
...
不同 provider 的 mappings 字段 `cwdPrefix` 共享，其他字段（`scmName` / `spaceKey` / etc）由 provider 自己定义。
...
- `knowledge/types.test.ts` — KnowledgeProvider 接口契约（fake provider）
- `knowledge/local-roles-provider.test.ts` — 内置 provider 行为
- `knowledge/depscope-provider.test.ts` — HTTP mock，缓存命中、失败降级、超时
- `knowledge/aggregator.test.ts` — 多 provider 并发 / timeout / 长度 cap
...
- `host_session_start` → KnowledgeProvider 聚合 → `additional_context` 注入
...
> 用户级强制要求：**每个用户流程必须有 e2e 测试**，攻击性视角，不做乐观假设。

技术栈：Playwright（驱动 Electron app）+ fake LarkChannel + fake Cursor hook 子进程注入器。

覆盖的用户流程（每条都有正常路径 + 至少 3 条攻击性变体）：

1. 首次启动 → hooks 安装 → 主窗口出现
2. Cursor 开 chat → menubar 显示活跃 → ChatDetail 实时事件流
3. PreToolUse Shell 拦截 → LocalChannel 弹窗 → 用户 Allow → 工具执行
4. PreToolUse 拦截 → Deny → Cursor 收到 deny + reason
5. Approval 超时（用户离开） → fallback `ask`，事件写 audit
6. `/allow!` → policy 写入 → 下次同范围自动通过
7. UI 内 "Mirror to Lark" → bind 创建 → 飞书 thread 收到种子消息
8. Lark 入站消息 → host_stop followup_message → Cursor chat 注入
9. Lark `/allow` → 桌面侧 LocalChannel pending 同时 settle
10. 用户主动 "沉淀为需求" → capture 流程 → 需求库出现
11. 用户主动 "沉淀为知识" → 角色 chunks 增加
12. doc-first 强制：未先 update_doc_first 直接 Write → 拦截
13. KnowledgeProvider 注入：在配置过 depscope mapping 的 cwd 开 chat → additional_context 出现 depscope 字段
14. KnowledgeProvider 失败降级：depscope 故意 500 → sessionStart 不阻塞，UI Settings 显示 unhealthy
15. campaign / cycle / task 主流程：init_workflow → create_tasks → complete_task 带 audit token → cycle 自动流转

攻击性变体覆盖：
- bridge socket 不存在 / 权限错 / 超时 / 客户端写半截 JSON
- Lark CLI 子进程 crash / 网络断 / 重连
- SQLite WAL 损坏 / 并发写
- hook 子进程 SIGKILL 中途
- depscope endpoint 慢响应、503、TLS 错误、JSON 损坏
- 多 chat 并发 + 多 pending approval + 多 channel 决策竞速
- 配置文件被外部进程改写 / 删除
- 用户 quit 时有 in-flight 审批

约定：每个流程在 `tests/e2e/<flow-name>/` 下一个目录，含 `happy.spec.ts` + `attack.spec.ts`。e2e 不允许打真实网络，所有外部依赖 mock。
...
> 用户级强制要求：**详细 log，便于用户反馈问题时提供完整上下文。**

### 19.5.1 设计原则

- 每个模块独立 logger（`src/logger.ts` 提供工厂，按模块名建子 logger）
- 结构化日志（JSON Lines），便于 grep / 上传
- 按 session 归档：每个 host_session 一个独立 log 文件方便 reproduce
- 敏感信息 redact（沿用 §13 / §19 的 redact 规则）
- 不同 level 不同文件：`main.log` / `error.log` / `events.jsonl` / `sessions/<id>.jsonl`

### 19.5.2 日志文件布局

```
~/.helm/logs/
├── main.log              # 主进程 info+ 文本
├── error.log             # 仅 error+
├── events.jsonl          # bridge / hook / channel 事件 JSON Lines
├── lark-listener.log     # lark-cli 子进程 stdout（继承）
├── lark-listener.err.log
├── sessions/
│   └── <host_session_id>.jsonl   # 单 session 全部事件
└── archive/              # 自动 rotate 后的归档
```

### 19.5.3 用户反馈 bundle

UI 在 Settings → Diagnostics 提供 "导出诊断包" 按钮：

- 打包：`config.json`（API key redact）、最近 N 天 logs、当前 SQLite schema 版本、`<name> doctor` 输出、最近 5 个 session 的 jsonl
- 输出 zip 文件，文件名含时间戳与 app 版本
- UI 提示用户："此 zip 包含 chat 事件记录，请检查后再分享"
- 不会自动上传到任何远程
...
```jsonc
{
  "ts": "2026-05-02T10:23:45.123Z",
  "level": "info",
  "module": "channel.lark",
  "host_session_id": "cursor-abc",  // optional
  "binding_id": "...",              // optional
  "event": "inbound_message",
  "msg": "received message from lark",
  "data": { /* 结构化字段，敏感信息 redact */ }
}
```

### 19.5.5 Log 轮转

- 单文件 ≥ 10MB → rotate 到 `archive/`
- archive 总占用 ≥ 500MB → 删最老
- 单 session jsonl 跟随 host_event_log 上限策略
...
### Phase 7.5：KnowledgeProvider 接口 + LocalRolesProvider（1 天）

- `src/knowledge/types.ts` 接口
- `LocalRolesProvider` 实现（包装现有 roles/chunks）
- 聚合器（并发 / timeout / cap）
- `host_session_start` 注入接入点
- 新增 MCP tool `query_knowledge` / `list_knowledge_providers`
- 测试
...
### Phase 13：DepscopeProvider reference 实现（1-2 天）

- HTTP client + cwd → scm_name 映射 + auth
- session context 渲染（ServiceSpec、call chain、HAR 覆盖）
- `knowledge_provider_cache` 持久化
- 失败降级 / healthcheck
- e2e：在 fixture 项目目录开 chat → additional_context 注入正确
- e2e 攻击：endpoint 500 / 慢 / TLS 错 → sessionStart 不阻塞

### Phase 14：e2e 测试套件全量补齐（贯穿前面所有 Phase，最后统一 review）

- §17.4 列出的 15 条用户流程，每条 happy + 至少 3 条 attack
- Playwright + Electron driver
- CI 强制运行（PR 必须 e2e 全绿）

### Phase 15：日志 / Diagnostics 包（1 天）

- §19.5 logger 工厂落地（之前各 Phase 已经按规范打 log）
- Settings → Diagnostics 导出按钮
- Log rotate
- redact 规则跑攻击性测试
...
- `RelayBackend` 接口落地（§11.6）
- `DepscopeRelay` / `CloudflareTunnelRelay` 实现
- PWA 项目（独立 React 仓库或 web/ 内子目录）
- Web Push 适配
- 多端配对 UI

## 22. Definition of Done（MVP-3）

- `pnpm typecheck` / `pnpm test` / `pnpm build` / `pnpm test:e2e` 全过
...
- KnowledgeProvider 抽象就绪，`LocalRolesProvider` 自动启用，`DepscopeProvider` reference 可启用
- 关闭 app 时无 orphan lark-cli 子进程
- §17.4 所有用户流程 e2e 套件 happy + attack 全部通过
- §19.5 日志规范全模块落地，Diagnostics 导出可用
...
- [ ] DepscopeProvider 是否 MVP-3 就并入（推荐：是，作为 reference KnowledgeProvider 实现，对内部用户立即有价值）
- [ ] Phase 2 RelayBackend 默认 backend 选哪个：DepscopeRelay 还是 CloudflareTunnel（推荐：内部构建走 Depscope，开源构建走 Cloudflare，编译期开关）
