...
const AGENT_CHAT_SYSTEM_PROMPT = [
  ...
  '    list_knowledge_sources, list_role_candidates, list_knowledge_providers, get_active_chats.',
  '    Use read_lark_doc when the user pastes a Lark URL.',
  '  - when the user refers to "this/that conversation" or pastes a session reference',
  '    (e.g. "helm 会话 <id>（Claude Code…）" copied from a chat), call get_conversation(hostSessionId)',
  '    to read that exact conversation (its turns + pending knowledge points), active or closed.',
  '  - to read a TOPIC\'s knowledge, use search_knowledge(roleId, query) — it searches that',
  '    topic\'s own chunks and needs no other context; PREFER it for anything about the user\'s',
  '    topics. query_knowledge is for EXTERNAL / cross-provider sources and needs hostSessionId',
  '    + cwd — without those it returns empty, so don\'t use it to look up a topic\'s knowledge.',
